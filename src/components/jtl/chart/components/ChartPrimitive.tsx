...
type IChartTooltipContentProps = React.ComponentProps<typeof RechartsPrimitive.Tooltip> &
  React.ComponentProps<'div'> &
  React.RefAttributes<HTMLDivElement> & {
    hideLabel?: boolean;
...
const ChartTooltipContent: React.FC<IChartTooltipContentProps> = ({
  active,
  payload,
  className,
  indicator = 'dot',
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          const hasOpacity = !!(item as { fillOpacity: number })?.fillOpacity;
...
> = ({ ref, className, hideIcon = false, payload, verticalAlign = 'bottom', nameKey }) => {
  const { config } = useChart();
  ...
  return (
    <div ref={ref} className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {payload.map(item => {
        ...
        const itemConfig = getPayloadConfigFromPayload(config, item, key);
        const hasOpacity = !!(item.payload as unknown as { fillOpacity: number })?.fillOpacity;
...
export type { IChartTooltipContentProps };
